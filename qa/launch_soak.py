#!/usr/bin/env python3
"""Detach one existing soak harness and record its process identity.
...
Example (a short check, not a long soak):
  python3 qa/launch_soak.py --python /path/to/runtime-linux/bin/python \
...
def launch(args: argparse.Namespace) -> dict:
    ...
    harness = Path(__file__).resolve().with_name("soak.py")
    if not harness.is_file():
        raise ValueError("The existing soak.py harness is missing")
    ...
    try:
        ...
        metadata = {"status": "STARTING", "created_at": datetime.now(timezone.utc).isoformat(),

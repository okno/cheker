...
Il registro degli estrattori aggiuntivi viene costruito da un bootstrap incluso nel wheel e congelato prima della lettura del documento nel worker. I risultati attraversano controlli comuni di tipo, completamento e budget; le normali regole dello scanner analizzano anche i segmenti prodotti dagli estrattori registrati. Il contesto del monitor comprende il fingerprint del registro, del codice confezionato e delle dipendenze dichiarate. Nella release consegnata con wheel `039e0fd8…` il registro aggiuntivo è vuoto. Il sorgente aggiornato il 15 settembre 2026 registra esplicitamente il profilo XLSX statico, destinato a un nuovo candidato e non attribuito alla release 039e. L’handler `xlsx_extractor.extract_xlsx` usa la dipendenza già presente `defusedxml` e gli stessi worker/report; non esegue Office, formule o OCR. Celle e metadati Transitional sono analizzati entro limiti; macro, media, dati esterni diversi dagli hyperlink e strutture non ispezionate interrompono la scansione. Il nuovo modulo rientra nel fingerprint del pacchetto e richiede la qualifica di una release distinta. [ESTRATTORI.md](ESTRATTORI.md) descrive il contratto per lo sviluppo.
